...
class JsonObjectNode extends JsonNode {
    constructor(property, value, options) {
        ...
        if (isDefined(value) && isDefined(value.type))
            this.type = value.type.charAt(0).toUpperCase() + value.type.substring(1);
        ...
        if (this.type === 'String' && this.datatype === Object && (this.value.pattern || this.value.format)) {
            if (this.value.format === undefined) this.isTestable = true;
            else this.isTestable = getPatternForFormat(this.value.format) !== undefined;
        }
    }
    ...
    get size() {
        if (this.type === 'Object' && this.value.properties !== undefined)
            return Object.keys(this.value.properties).length;
        else if (this.type === 'Enum') return this.value.enum.length;
        return Object.keys(this.value).length;
    }

    get htmlNode() {
        const count = !['Object', 'Enum'].includes(this.type) ? undefined : this.size;
